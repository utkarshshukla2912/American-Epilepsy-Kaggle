...
filenum=0
	...
	#List of lists
with open('Dog5_train_sample_feature_ext.csv', 'wb') as f:
	writer = csv.writer(f)
	...
	for filename in os.listdir("../../files/test"):
		filenum = filenum + 1
		print "File:", filenum
		print "File:", filename
		element_writer=list()
		if filename.endswith(".mat"):
			fwr=list()
			feature_writer=list()
			file_read = si.loadmat('../../files/test/'+filename)
			...
			if ictal != 'None' and 'test' not in ictal:
				...
				for channel in all_channels:
					...
					feature_writer=list()
					chunks = np.array_split(channel, 20)
					print len(chunks)
					chunk_no = 1
					for chunk in chunks:
						...
						stats_channel=Stats(chunk)#Four feature cols
						for i in range(len(stats_channel)):
							feature_writer.append(stats_channel[i])
		
						hurst_channel=hurstFD(chunk)              #One feature col
						feature_writer.append(hurst_channel)    


						pfd_channel=pfd(chunk)                    #One feature col
						feature_writer.append(pfd_channel)


						hjorth_channel=hjorthFD(chunk)            #One feature col
						...
						katz_channel=katzFD(chunk)                #One feature col
						feature_writer.append(katz_channel)



						higuchi_channel=higuchiFD(chunk)#One feature col
						...
						shannon_channel=shannon_entropy(chunk)    #One feature col
						feature_writer.append(shannon_channel)

						moments_channel=Moments(chunk)
						for val in moments_channel:
							feature_writer.append(val)
					


						psd_channel=power_spectral_density(chunk)
